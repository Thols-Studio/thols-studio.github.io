  ...
  state.playlists.forEach(pl => {
    ...
    btn.innerHTML = `
      ...
      <span class="col-actions" id="pl-actions-${pl.id}">
        <button class="col-action-btn danger" onmousedown="event.stopPropagation();deletePlaylist('${pl.id}')" title="Delete playlist">
    ...
    btn.onclick = () => filterByCollection(`playlist:${pl.id}`, btn);
    // hover show/hide actions
    btn.addEventListener('mouseenter', () => { btn.querySelector('.col-actions').style.opacity = '1'; });
    btn.addEventListener('mouseleave', () => { btn.querySelector('.col-actions').style.opacity = ''; });
    list.appendChild(btn);
  });
}
...
function renderCards() {
  ...
  if (currentFilter !== 'all' && currentFilter !== 'recent' && currentFilter !== 'watched') {
    ...
    state.collections.forEach(col => {
      ...
      html += buildCollectionBlock(col.id, col.name, col.color, colVids, isFirstBlock);
      isFirstBlock = false;
    });
    ...
    if (vidColIds.has('__none__')) {
      ...
      if (uncollected.length) {
        html += buildCollectionBlock('__none__', 'Uncollected', 'var(--text-dim)', uncollected, isFirstBlock);
      }
    }
  }
...
function buildCollectionBlock(colId, colName, colColor, colVids, isFirst) {
  ...
  html += `<div class="group-header" onclick="toggleGroup('col-${colId}')">
    <span class="group-chevron collapsed" id="chev-col-${colId}">
  ...
  html += `<div class="group-body collapsed" id="group-col-${colId}">`;
  html += renderGroupedByTitle(colVids, colId, colColor, true);
  html += `</div></div>`;
  return html;
}

function renderGroupedByTitle(vids, colId, colColor, nested = false) {
  ...
  groupNames.forEach(gName => {
    ...
    html += `<div class="group-section">
      ...
      <div class="group-body collapsed" id="group-${gKey}">
...
function toggleWatched(id) {
  ...
  if (v) { v.watched = !v.watched; save(); renderSidebar(); renderCards(); }
}

function markWatched(id) {
  const v = state.videos.find(v => v.id === id);
  if (v && !v.watched) { v.watched = true; save(); renderSidebar(); }
}
